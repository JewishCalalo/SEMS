...
<?php endif; ?>
                ...
                <div class="p-6">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div>
                            ...
                            <p class="text-gray-600 mt-1">View and manage equipment information</p>
                        </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2">
                    ...
                    <!-- Manager guide moved to match admin placement (outside container below) -->
                </div>
                ...
                <div class="lg:col-span-1">
                    ...
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        <div class="p-6">
                            <h4 class="text-sm font-medium text-gray-900 mb-3">Equipment Actions</h4>
                            <div class="space-y-2">
                                <button type="button" 
                                    ...
                                    <i class="fas fa-plus mr-2"></i>Add Instances
                                </button>
                                <button type="button" 
    ...
    <!-- Centered Deletion & Retirement Rules just below Equipment Details (outside containers to center like admin) -->
    <div class="mt-4">
                    ...
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div class="flex items-center gap-2">
                                <button type="button" onclick="bulkRetireInstancesMgr()" class="px-4 py-2 bg-yellow-600 text-white rounded-md text-sm font-semibold hover:bg-yellow-700 transition">Retire Selected</button>
                                <button type="button" onclick="bulkDeleteInstancesMgr()" class="px-4 py-2 bg-black text-white rounded-md text-sm font-semibold hover:bg-gray-900 transition">Delete Selected</button>
                            </div>
                        </div>
                    </div>
                    
                    <!-- Removed duplicate rules toggle (manager) to match single guide behavior -->

                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-red-600 sticky top-0 z-10">
                                <tr>
                                    <th class="px-6 py-3 text-left text-xs font-bold text-white uppercase tracking-wider w-12"><input type="checkbox" onclick="toggleSelectAllInstancesMgr(this)" class="w-4 h-4 text-red-600 focus:ring-red-500 border-gray-300 rounded"></th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <?php $__empty_1 = true; $__currentLoopData = $equipment->instances; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $instance): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-6 py-4 whitespace-nowrap text-sm"><input type="checkbox" class="instance-checkbox-mgr w-4 h-4 text-red-600 focus:ring-red-500 border-gray-300 rounded" value="<?php echo e($instance->id); ?>"></td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            <span class="inline-flex px-2 py-0.5 text-xs rounded-full
                                                <?php echo e($instance->condition === 'excellent' ? 'bg-green-100 text-green-800' :
                                                   ($instance->condition === 'good' ? 'bg-blue-100 text-blue-800' :
                                                   ($instance->condition === 'fair' ? 'bg-yellow-100 text-yellow-800' :
                                                   ($instance->condition === 'needs_repair' ? 'bg-orange-100 text-orange-800' :
                                                   ($instance->condition === 'damaged' ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-800'))))); ?>">
                                                <?php echo e(ucfirst(str_replace('_',' ', $instance->condition))); ?>

                                            </span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            <?php
                                                $status = $equipment->getInstanceReservationStatus($instance->id);
                                                $isAvailable = $status === 'Available';
                                            ?>
                                            <?php echo e($isAvailable ? 'Yes' : 'No'); ?>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            ...
                                            <?php if($status === 'Reserved'): ?>
                                                <span class="inline-flex px-2 py-0.5 text-xs rounded-full bg-purple-100 text-purple-800">Reserved</span>
                                            <?php elseif($status === 'Borrowed'): ?>
                                                <span class="inline-flex px-2 py-0.5 text-xs rounded-full bg-red-100 text-red-800">Borrowed</span>
                                            <?php elseif($status === 'Unavailable'): ?>
                                                <span class="inline-flex px-2 py-0.5 text-xs rounded-full bg-gray-100 text-gray-800">Unavailable</span>
                                            <?php else: ?>
                                                <span class="inline-flex px-2 py-0.5 text-xs rounded-full bg-green-100 text-green-800">Available</span>
                                            <?php endif; ?>
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            <?php
                                                $reservations = $instance->getCurrentReservationDates();
                                            ?>
                                            <?php if($reservations->count() > 0): ?>
                                                <?php $__currentLoopData = $reservations; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $reservation): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                    <div class="text-xs">
                                                        <span class="text-black"><?php echo e(\Carbon\Carbon::parse($reservation['borrow_date'])->format('M d')); ?> - <?php echo e(\Carbon\Carbon::parse($reservation['return_date'])->format('M d')); ?></span>
                                                        <span class="ml-1 px-1 py-0.5 rounded text-xs <?php echo e($reservation['is_current'] ? 'bg-red-100 text-red-800' : ($reservation['is_future'] ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-600')); ?>">
                                                            <?php echo e($reservation['is_current'] ? 'Current' : ($reservation['is_future'] ? 'Future' : 'Past')); ?>
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.confirmation-modal','data' => ['id' => 'delete-equipment-modal','title' => 'Delete Equipment','message' => 'Are you sure you want to delete \''.e($equipment->display_name).'\'? This action cannot be undone and will remove all associated data.','confirmText' => 'Delete Equipment','cancelText' => 'Cancel']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['id' => 'delete-equipment-modal','title' => 'Delete Equipment','message' => 'Are you sure you want to delete \''.e($equipment->display_name).'\'? This action cannot be undone and will remove all associated data.','confirmText' => 'Delete Equipment','cancelText' => 'Cancel']); ?>
<?php echo $__env->renderComponent(); ?>
<?php endif; ?>
...
<?php endif; ?>
    ...
    <script>
        ...
        function toggleSelectAllInstancesMgr(src){
            document.querySelectorAll('.instance-checkbox-mgr').forEach(cb => cb.checked = src.checked);
        }

        async function bulkDeleteInstancesMgr(){
            const selected = Array.from(document.querySelectorAll('.instance-checkbox-mgr:checked')).map(cb => cb.value);
            ...
            const checks = Array.from(document.querySelectorAll('.instance-checkbox-mgr:checked'));
            ...
            if(failed===0){
                Swal.fire({
                    ...
                    html:`
                        <div class=\"bg-gradient-to-r from-green-600 to-emerald-600 text-white p-4 -m-6 mb-4 rounded-t-lg\"> <h2 class=\"text-xl font-bold\">Deletion Successful</h2></div>
                        <div class=\"text-center\">
                            <div class=\"mx-auto w-12 h-12 rounded-full bg-green-100 flex items-center justify-center mb-3\"> <svg class=\"w-6 h-6 text-green-600\" fill=\"none\" stroke=\"currentColor\" viewBox=\"0 0 24 24\"><path stroke-linecap=\"round\" stroke-linejoin=\"round\" stroke-width=\"2\" d=\"M5 13l4 4L19 7\"/></svg></div>
                ...
                Swal.fire({
                    buttonsStyling:false,
                    html:`
                        <div class=\"bg-gradient-to-r from-red-600 to-pink-600 text-white p-4 -m-6 mb-4 rounded-t-lg\"><h2 class=\"text-xl font-bold\">Some deletions failed</h2></div>
                        <div class=\"text-left\">
        ...
        async function bulkRetireInstancesMgr(){
            const selected = Array.from(document.querySelectorAll('.instance-checkbox-mgr:checked')).map(cb => cb.value);
            if(selected.length===0){ Swal.fire({icon:'info', title:'No instances selected', text:'Select at least one instance.', customClass:{popup:'swal-custom-popup'}}); return; }
            ...
            let success=0, failed=0;
            for(const id of selected){
                try{ const resp = await fetch(`<?php echo e(url('/instances')); ?>/${id}/retire`, {method:'POST', headers:{'X-CSRF-TOKEN':'<?php echo e(csrf_token()); ?>','Accept':'application/json'}}); if(resp.ok){ success++; } else { failed++; } }
                catch(e){ failed++; }
            }
            const html = failed===0 ? `
...
<?php endif; ?>
<?php /**PATH C:\SEMSv25\SEMS\resources\views\manager\equipment-management\show.blade.php ENDPATH**/ ?>