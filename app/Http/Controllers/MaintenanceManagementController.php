...
class MaintenanceManagementController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $viewPath = $this->getViewPath('maintenance-management', 'index');
    ...
    public function show(Equipment $equipment, Request $request)
    {
        ...
        $viewPath = $this->getViewPath('maintenance-management', 'show');
    ...
    public function createRecord(Equipment $equipment)
    {
        ...
        $viewPath = $this->getViewPath('maintenance-management', 'create-record');
    ...
    public function editRecord(MaintenanceRecord $maintenanceRecord)
    {
        ...
        $viewPath = auth()->user()->isAdmin() ? 'admin.maintenance-management.edit-record' : 'manager.maintenance-management.edit-record';
    ...
    public function reports()
    {
        try {
            ...
            $viewPath = auth()->user()->isAdmin() ? 'admin.maintenance-management.reports' : 'manager.maintenance-management.reports';
            return view($viewPath, compact(
                'monthlyCosts', 
                'maintenanceByType', 
    ...
    public function generatePDF(Request $request)
    {
        try {
            ...
            \Log::info('Maintenance PDF Generation Request - Method: ' . $request->method());
            \Log::info('Maintenance PDF Generation Request - URL: ' . $request->url());
            \Log::info('Maintenance PDF Generation Request - All data: ' . json_encode($request->all()));
            ...
            // Debug: Log the maintenance records count and filters
            \Log::info('Maintenance PDF Generation - Records count: ' . $maintenanceRecords->count());
            \Log::info('Maintenance PDF Generation - Filters: ' . json_encode($filters));
            ...
            \Log::info('Maintenance PDF Generation - Success, filename: ' . $filename);
    ...
    public function discarded(Request $request)
    {
        ...
        $viewPath = auth()->user()->isAdmin() ? 'admin.maintenance-management.discarded' : 'manager.maintenance-management.discarded';
