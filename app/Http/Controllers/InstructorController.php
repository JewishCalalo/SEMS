...
class InstructorController extends Controller
{
        ...
        try {
            ...
            try {
                ...
                if ($shouldUseSignature && !$proceedDuplicate) {
                    $reservationData['signature'] = $signature;
                }
    ...
    public function exportReport(Request $request)
    {
        ...
        if ($format === 'pdf') {
            return $this->generatePdfReport($data, $reportType, $startDate, $endDate);
        } elseif ($format === 'excel') {
            return $this->generateExcelReport($data, $reportType, $startDate, $endDate);
        }
     ...
     * Generate PDF report
     */
    private function generatePdfReport($data, $reportType, $startDate, $endDate)
    {
        // Build dataset based on type
        if ($reportType === 'reservations') {
            $reservations = collect($data['reservations']);
            $pdf = \App\Services\PDFService::generateReservationReport($reservations, [
                'date_from' => $startDate,
                'date_to' => $endDate,
                'owner' => auth()->user()->name,
                'scope' => 'Instructor personal reservations'
            ]);
            return $pdf->stream('instructor-reservations-' . now()->format('Ymd_His') . '.pdf');
        }
        ...
        if ($reportType === 'incidents') {
            ...
            $pdf = \Barryvdh\DomPDF\Facade\Pdf::loadHTML($html)->setPaper('a4', 'landscape');
            return $pdf->stream('instructor-incidents-' . now()->format('Ymd_His') . '.pdf');
        }
    }
     ...
     */
    private function generateExcelReport($data, $reportType, $startDate, $endDate)
    {
        // Produce a CSV stream compatible with Excel
        $filename = 'instructor-' . $reportType . '-' . now()->format('Ymd_His') . '.csv';
        $headers = [
        ...
        $callback = function() use ($data, $reportType) {
            ...
            if ($reportType === 'reservations') {
                fputcsv($out, ['Code', 'Status', 'Borrow Date', 'Return Date', 'Items', 'Created']);
                foreach ($data['reservations'] as $r) {
                    fputcsv($out, [
                        $r->reservation_code,
                        $r->status,
                        optional($r->borrow_date)->format('Y-m-d'),
                ...
                fputcsv($out, ['Code', 'Type', 'Severity', 'Status', 'Date']);
