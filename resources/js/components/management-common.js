...
function createReportModal(title, formAction, formFields) {
    ...
    Swal.fire({
        ...
        didOpen: () => {
            ...
            const typeEl = formEl?.querySelector('select[name="equipment_type"]');
            const typeGroup = typeEl ? typeEl.closest('div') : null;
            const syncTypeVisibility = () => {
                if (!typeEl || !typeGroup) return;
                const hasCategory = Boolean(categoryEl && categoryEl.value);
                typeGroup.style.display = hasCategory ? '' : 'none';
            };
            if (categoryEl && typeEl && typeGroup) {
                syncTypeVisibility();
                categoryEl.addEventListener('change', syncTypeVisibility);
            }
            if (formatEl) {
            ...
            window.generateReport = function() {
                ...
                let url;
                if (format === 'excel') {
                    url = form.action.replace('generate-pdf', 'export/excel');
                } else if (format === 'csv') {
                    url = form.action.replace('generate-pdf', 'export/csv');
                } else {
                    url = form.action; // PDF
                }
