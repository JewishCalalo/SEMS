                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                @foreach($incident->attachments as $attachment)
                                    ...
                                    <div class="border border-gray-200 rounded-lg p-4">
                                        <button type="button" class="w-full text-left flex items-center space-x-3 group" @if($isImage) onclick="showImageModal('{{ $url }}')" @else onclick="window.open('{{ $url }}','_blank')" @endif>
                                            <div class="flex-shrink-0">
                                                @if($isImage)
                                                    <img src="{{ $url }}" alt="{{ $attachment['filename'] }}" class="w-12 h-12 object-cover rounded shadow-sm group-hover:opacity-90">
                                                @else
