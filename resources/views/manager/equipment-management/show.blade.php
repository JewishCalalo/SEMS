<x-app-layout>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse($equipment->instances as $instance)
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-6 py-4 whitespace-nowrap text-sm"><input type="checkbox" class="instance-checkbox-mgr w-4 h-4 text-red-600 focus:ring-red-500 border-gray-300 rounded" value="{{ $instance->id }}"></td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            <span class="inline-flex px-2 py-0.5 text-xs rounded-full
                                                {{ $instance->condition === 'excellent' ? 'bg-green-100 text-green-800' :
                                                ...
                                                {{ ucfirst(str_replace('_',' ', $instance->condition)) }}
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            @php
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            ...
                                            @if($reservations->count() > 0)
                                                @foreach($reservations as $reservation)
                                                    <div class="text-xs">
                                                        <span class="text-black">{{ \Carbon\Carbon::parse($reservation['borrow_date'])->format('M d') }} - {{ \Carbon\Carbon::parse($reservation['return_date'])->format('M d') }}</span>
                                                        <span class="ml-1 px-1 py-0.5 rounded text-xs {{ $reservation['is_current'] ? 'bg-red-100 text-red-800' : ($reservation['is_future'] ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-600') }}">
                                                            {{ $reservation['is_current'] ? 'Current' : ($reservation['is_future'] ? 'Future' : 'Past') }}
                                                        </span>
                                                    </div>
                                                @endforeach
                                            @else
    ...
    <x-confirmation-modal 
        ...
        message="Are you sure you want to delete '{{ $equipment->display_name }}'? This action cannot be undone and will remove all associated data."
    ...
    <script>
        ...
        async function bulkRetireInstancesMgr(){
            ...
            let success=0, failed=0;
            for(const id of selected){
                try{ const resp = await fetch(`{{ url('/instances') }}/${id}/retire`, {method:'POST', headers:{'X-CSRF-TOKEN':'{{ csrf_token() }}','Accept':'application/json'}}); if(resp.ok){ success++; } else { failed++; } }
                catch(e){ failed++; }
            }
            const html = failed===0 ? `
